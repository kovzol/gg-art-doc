...
\label{par:Thales}
Here we want to explore the possible perpendicularity of lines $CA$, $CB$, where $C$ is a point on a circle, while $AB$ is a diameter thereof. We can proceed as follows:
\begin{center}
...
\paragraph{Example}
Given a triangle $ABC$, and the circle having $AB$ as diameter, find the locus of $C$ such that $CA^2+ CB^2=AB^2$ (a converse of Pythagoras' Theorem).
\begin{center}
...
investigations that are not supported by GeoGebra ART at the moment. See \cite{BotanaRecio2017}
...
In many traditional math classes, Thales' circle theorem is stated in an explicit form: if $C$ is on a semicircle, the segments $g$ and $h$ are perpendicular (see Section \ref{par:Thales} on page
